...
class IndicatorDataset(object):
    ...
    def dates(self, use_datetime=False):
        ...
        """
        dates = []
        for country_data in list(self.as_dict().values()):
            for date in list(country_data.keys()):
...
class IndicatorAPI(object):
    ...
    def print_codes(self, results, search=None, search_key=None):
        ...
        for k, v in sorted(list(results.items()), key=natural_keys):
            # Value will either be a dict or string
            if hasattr(v, "get"):
                main_value = v.get("name", v.get("value", v))
            else:
                main_value = v
            print("{0:30} {1}".format(k, main_value))
    ...
    def search_results(self, regexp, results, key=None):
        ...
        if key:
            for k, v in list(results.items()):
                row_string = "{0} {1}".format(k, v[key])
                if compiled_re.search(row_string):
                    search_matches[k] = v
        else:
            for k, v in list(results.items()):
                row_string = "{0} {1}".format(k, v)
                if compiled_re.search(row_string):
                    search_matches[k] = v
    ...
    def _generate_indicators_url(
        ...
        **kwargs):
        ...
        """
        kwargs = dict([(k.lower(), v) for k, v in list(kwargs.items())])
        ...
        if dataset_params:
            ...
            if all(key not in list(kwargs.keys()) for key in ["mrv", "date"]):
                kwargs["mrv"] = 1
        ...
        sorted_kwargs = sorted([(k, v) for k, v in list(kwargs.items())])
